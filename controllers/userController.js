...
const updateUser = async (req,res)=>{
    try{
        ...
        if(isUser){
            ...
            return res.status(201).json({message: `user ${isUser.firstName} successfully updated`})
        };
        return res.status(400).json({message: `error updating user ${firstName}`})
...
const deleteUser = async (req,res)=>{
    try{
        ...
        if(destroyUser){
            return res.status(201).json({message: ` user ${isUser.firstName} deleted successfully`}) 
        }
        return res.status(400).json({message: `error deleting user ${isUser.firstName}`})
...
const createAccount = async (req, res)=>{
    try{
        ...
        const acctNumber = await Math.floor((Math.random()* 9000000000) + 1000000000).toString();
        ...
        return res.status(201).json(await{message:"account created successfully", newAccount})
...
const makeCredit = async (req,res)=>{
    try{
        ...
        isAccount.accountBalance = parseFloat(isAccount.accountBalance) + parseFloat(transactionAmount) ;
        ...
        return res.status(201).json({message:"money transferred successfully", credit})
...
const generateAcctStatement = async (req,res)=>{
    try{
        ...
        const isAccount = await transactionTable.findAll({where: {accountId : req.params.id},createdAt: {
...
module.exports = {createUser,users,updateUser,deleteUser,createAccount,makeCredit,makeDeposit, generateAcctStatement,transactHistory};